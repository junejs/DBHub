...
- 作为安全管理员，我希望每一次导出都被强审计，并能按环境限制导出规模（v1 审批流延后到 v2）。
...
- （v2）**来自审批单**：审批通过的批量导出任务，可走 ADMIN 连接（见 [18 §1.7](./18-roadmap.md)）。
...
- **产物 ZIP 密码仅由用户在发起时自设**（强密码校验）；平台**不存储、不下发**密码，下载时按用户提供密码流式解密校验，密码错误 → `403 EXPORT_PASSWORD_INVALID`（见 [12 §4.3](./12-api-contract.md)）。v1 不提供"系统生成密码 + 安全渠道下发"路径（无可用安全通道）。
...
| **环境差异（重要）** | 导出护栏按环境差异化：行数上限取 `environment_policies.export_max_rows`；结果字节上限、并发查询数等运行时策略入 `environment_policies.settings` 或 `settings`。v1 不引入导出审批。 |
