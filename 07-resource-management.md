...
Workspace（工作区，单组织自部署下为单一固定实例）
  │
  ├── Environment（一等策略维度：dev/test/stage/prod；实例必标，库可继承/覆盖）—— 驱动访问控制/护栏
  │
...
- **Project 是逻辑隔离边界**：跨 Project 默认隔离（见 [02 §2.3](./02-permission-and-access.md)），Project A 的成员看不到也访问不了 Project B 的实例与库。
- **Instance 与 Database 是持久化的一等资源**（关系表行）。
- **Schema / Table / Column 不是独立行**，而是作为嵌套元数据结构（`DatabaseSchemaMetadata`）整体存储 + 缓存。
- **Environment = 一等策略维度（重要）**：每个 Instance **必须标注** environment（dev/test/stage/prod 等）；Database 可显式覆盖，否则继承其实例的环境（`effective_environment`）。环境不只是展示标签，而是**驱动访问控制（结构化条件按环境）、查询/导出护栏**的策略维度，由 `environment_policies` 表按环境差异化配置（见 [02 §3.2](./02-permission-and-access.md)、[10 environment_policies](./10-data-model.md)）。环境的增删与策略配置由 `securityAdmin`/`workspaceAdmin` 管理（平台级），实例上的标注由项目角色填写。
...
- SSL/TLS、SSH 隧道（PostgreSQL 连接参数）。
...
- schema 元数据（结构树）在内存合并为统一可查询视图（`DatabaseMetadata`），供查询/补全使用。
- 大小写敏感性按 PostgreSQL 规则处理（标识符折叠为小写）。
- （列级 Catalog 标注：semantic_type/classification/labels 属脱敏能力，延后 v2。）
...
- **资源树**：按 **Project**（团队隔离边界）→ Instance → Database → Schema → Table/View → Column 层级展示；用户只能看到自己所属 Project 下的实例与库（跨 Project 默认不可见，见 [02 §2.3](./02-permission-and-access.md)）。
...
- 表详情：列定义、索引、外键、行数/大小、DDL、数据预览（受权限）。
- 支持结构搜索（按表名/列名）。
...
v1 仅 PostgreSQL，所有能力（查询、结构同步、自动补全）均支持，无需能力矩阵或降级逻辑。未来多引擎时再引入。

---

## 9. 功能范围

- 实例注册/测试连接/更新/删除/数据源管理（**当前版本支持 PostgreSQL**）。
- 库自动发现 + 归属初始项目。
- 后台元数据同步 + 缓存 + 手动刷新。
- 资源树浏览 + 结构详情 + DDL。
- 环境与项目管理。

**可选增强（不在 v1 范围）：** 列级 Catalog 标注（见 [18 §1.1](./18-roadmap.md)）、结构变更对比（diff）、结构变更历史、库结构搜索增强、跨实例资源视图、更多引擎（见 [18 §1.4](./18-roadmap.md)）。
