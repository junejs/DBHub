...
- 让用户安全地**分享** SQL 给他人，且**分享只传递「SQL 文本」，不传递任何数据访问权限**——执行时仍按接收者本人的权限/审计约束。
...
- **我的收藏**：侧边栏聚合视图，按最近使用/收藏时间/名称排序。
- **一键打开**：点击 → 在工作台打开对应 Worksheet（带上关联库），可直接执行或编辑后另存。
- **搜索**：按标题、SQL 内容、关联库过滤。
- **归属**：收藏为**用户私有**，他人不可见；取消收藏不影响底层 Worksheet。
- （文件夹/分组、置顶、备注延后；v1 仅收藏。）
...
- 作为安全管理员，我希望分享**只传 SQL、不传权限**——同事执行时仍走他自己的权限。
...
|---|---|
| `PRIVATE` | 仅创建者可见（默认） |
| `PROJECT` | 同项目成员可查看/执行/收藏 |

- 改可见性即「分享」：把 Worksheet 从 PRIVATE 改为 PROJECT，项目内同事即刻在「项目查询」里看到并执行（仍按**执行者本人**权限/审计）。
- （`PROJECT_WRITE` 协作编辑、`LINK` 带 token 分享链接延后；v1 以项目可见性覆盖团队共享。）
...
- 接收者打开分享的 Worksheet 并执行时：
  - 走**接收者本人**的 IAM 权限（`db.sql.select` + 库/表结构化条件 + 项目隔离）。
  - 若接收者无权访问该库/表 → 执行被拒绝（提示无权限），但可查看 SQL 文本。
- 分享者把 SQL 改写为「越权查询」也无法让接收者越权——权限校验在执行时按接收者身份强制。
- 分享行为本身**审计**：谁把哪个 Worksheet 的可见性改成了什么范围。

### 4.4 协作增强（延后）
- 评论/备注、派生另存为模板等协作能力延后。v1 支持基本的「另存为」自己的 Worksheet。
...
| `FavoriteWorksheet` | `POST .../worksheets/{id}:favorite` | 收藏 |
| `UnfavoriteWorksheet` | `POST .../worksheets/{id}:unfavorite` | 取消收藏 |
| `ListMyFavorites` | `GET .../me/favorites` | 我的收藏列表（支持过滤/排序） |
...
| 审计 | 收藏行为不审计（个人操作）；**改可见性（分享）需审计** |
| 可见性边界 | PRIVATE 仅本人；PROJECT 受项目成员关系约束 |

---

## 7. 非功能需求
- 收藏列表打开 P95 ≤ 300ms（个人数据，量小）。

---

## 8. 功能范围
- 收藏（一键收藏、我的收藏列表、搜索、一键打开）。
- 分享（基于 Worksheet 可见性：PRIVATE / PROJECT）。
- 安全边界：执行按接收者本人权限；改可见性审计。

**可选增强（不在 v1 范围，见 [18 §1.8](./18-roadmap.md)）：** 文件夹/分组/置顶/备注、PROJECT_WRITE 协作编辑、带 token 的分享链接(LINK)、评论、跨项目分享。
